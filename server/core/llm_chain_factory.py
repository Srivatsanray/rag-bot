...
CE_LOW_CONFIDENCE_THRESHOLD = 0.0
...
MULTI_QUERY_N_RESULTS = 3
MULTI_QUERY_CANDIDATE_CAP = 12
...
SYSTEM_PROMPT = (
    ...
    "If the context covers the question partially, answer what you can and clearly state "
    "which aspects are not covered in the uploaded documents. "
    "Do not make up information or use knowledge outside the provided context."
)

HYDE_PROMPT = (
    "You are a technical document expert. "
    "Given the question below, write a short hypothetical passage (2-3 sentences) "
    "that would appear in a document and would directly answer it. "
    "Write only the passage text. No explanation, no preamble, no meta-commentary."
)

DECOMPOSE_PROMPT = (
    "You are a search query decomposer. "
    "Given a user question, output 3 to 4 short focused search queries that together "
    "cover all distinct sub-topics of the question. "
    ...
    'Example: {"queries": ["encoder stack transformer", "decoder stack transformer", '
    '"multi-head attention mechanism", "positional encoding transformer"]}'
)
...
def _generate_hypothetical_answer_sync(query: str) -> tuple[str, bool]:
    """
    Blocking Groq call. Always called via asyncio.to_thread so it never
    runs on the event loop directly.
...
async def _generate_hypothetical_answer(query: str) -> tuple[str, bool]:
    """Async wrapper — offloads the blocking Groq call to a thread pool."""
    return await asyncio.to_thread(_generate_hypothetical_answer_sync, query)


def _decompose_query_sync(query: str) -> list[str]:
    """
    Blocking Groq call. Always called via asyncio.to_thread.
...
async def _decompose_query(query: str) -> list[str]:
    """Async wrapper — offloads the blocking Groq call to a thread pool."""
    return await asyncio.to_thread(_decompose_query_sync, query)


def _find_similar_chunks_sync(
    hyde_query: str, original_query: str, n_results: int
) -> list[dict]:
    """
    Synchronous wrapper around find_similar_chunks for use with asyncio.to_thread.
    Qdrant's local client and the cross-encoder inside find_similar_chunks are
    both blocking and must not run on the event loop directly.
    """
...
async def _retrieve_for_sub_query(sub_q: str) -> list[dict]:
    """
    Run HyDE generation and retrieval for a single sub-query.
    Both steps are offloaded to the thread pool so this coroutine is safe
    to run concurrently with other sub-query coroutines via asyncio.gather.
    """
    hypothetical, _ = await _generate_hypothetical_answer(sub_q)
    return await asyncio.to_thread(
        _find_similar_chunks_sync, hypothetical, sub_q, MULTI_QUERY_N_RESULTS
    )


async def _retrieve_multi_query(query: str) -> list[dict]:
    """
    Decompose the query, then fan out HyDE + retrieval for all sub-queries
    concurrently using asyncio.gather. Deduplicates the merged candidate pool
    by chunk identity, then reranks against the original query with the
    cross-encoder.
    ...
    The cross-encoder rerank is offloaded to a thread pool because it is a
    CPU-bound forward pass that would block the event loop if run inline.
    """
    sub_queries = await _decompose_query(query)

    # Fan out: all sub-query HyDE + retrieval calls run concurrently.
    all_results: list[list[dict]] = await asyncio.gather(
        *[_retrieve_for_sub_query(sub_q) for sub_q in sub_queries]
    )
    ...
    def _rerank(pairs: list[tuple]) -> list[float]:
        return cross_encoder.predict(pairs).tolist()

    pairs = [(query, chunk["chunk_text"]) for chunk in candidates]
    scores: list[float] = await asyncio.to_thread(_rerank, pairs)
...
async def _retrieve_single_query(query: str) -> tuple[list[dict], bool]:
    """
    Standard single-pass HyDE retrieval for narrow factual questions.
    Both HyDE generation and retrieval are offloaded to the thread pool.
    Returns (chunks, hyde_succeeded).
    """
    hypothetical, hyde_succeeded = await _generate_hypothetical_answer(query)
    chunks = await asyncio.to_thread(
        _find_similar_chunks_sync, hypothetical, query, SINGLE_QUERY_N_RESULTS
    )
...
def is_low_confidence(chunks: list[dict]) -> bool:
    ...
    return top.get("score", 0.0) < LOW_CONFIDENCE_THRESHOLD
...
async def generate_answer(query: str) -> dict:
    ...
    Broad queries go through multi-query decomposition with concurrent retrieval.
    Narrow queries go through single-pass HyDE retrieval.
    The final LLM call is also offloaded to the thread pool since the Groq
    client is synchronous.

    Your FastAPI route handler must await this coroutine:
    ...
    if broad:
        chunks = await _retrieve_multi_query(query)
        hyde_succeeded = (
            None  # HyDE runs per sub-query internally; not meaningful as a single bool
        )
    else:
        chunks, hyde_succeeded = await _retrieve_single_query(query)
    ...
    # llama-4-scout (and some reasoning models) emit <think>...</think> blocks
